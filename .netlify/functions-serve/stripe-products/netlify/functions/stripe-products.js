...
module.exports = __toCommonJS(stripe_products_exports);
...
var isStripeConfigured = stripeSecretKey && stripeSecretKey !== "sk_test_REPLACE_WITH_YOUR_ACTUAL_SECRET_KEY" && !stripeSecretKey.includes("TYooMQauvdEDq54NiTphI7jx") && (stripeSecretKey.startsWith("sk_test_") || stripeSecretKey.startsWith("sk_live_"));
var stripe = null;
...
var handler = async (event, context) => {
  ...
  try {
    ...
    const transformedProducts = products.data.filter((product) => product.default_price).map((product) => {
    ...
    const message = transformedProducts.length > 0 ? `Successfully loaded ${transformedProducts.length} products from Stripe` : "No products found in Stripe - add products to your Stripe dashboard";
...
//# sourceMappingURL=stripe-products.js.map
