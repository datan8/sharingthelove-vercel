...
console.log("\u{1F50D} Environment Debug Info:");
...
var backendAccountId = STRIPE_SECRET_KEY.substring(8, 25);
console.log("\u{1F511} Backend Stripe Key Account ID:", backendAccountId);
var stripe;
try {
  stripe = new stripe_esm_node_default(STRIPE_SECRET_KEY, {
...
var handler = async (event, context) => {
  ...
  try {
    ...
    const { amount, currency = "nzd", metadata = {}, items } = JSON.parse(event.body);
    if (!amount || amount <= 0) {
      return {
        statusCode: 400,
        headers,
    ...
    console.log(`\u{1F680} Creating NEW payment intent for ${finalAmount} cents (${currency})`);
    console.log("\u{1F4E6} Items:", items.map((item) => `${item.name} x${item.quantity} @ ${item.price}`).join(", "));
    console.log("\u{1F511} Using Stripe key account:", STRIPE_SECRET_KEY.substring(8, 25));
    ...
    console.log(`\u{1F50D} PaymentIntent account: ${piAccountId}`);
    console.log(`\u{1F50D} Backend account ID: ${backendAccountId}`);
    console.log(`\u{1F50D} Account IDs match: ${backendAccountId === piAccountId}`);
    console.log(`\u{1F4CA} PaymentIntent status: ${paymentIntent.status}`);
    console.log(`\u{1F4B0} Amount: ${paymentIntent.amount} ${paymentIntent.currency}`);
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({
